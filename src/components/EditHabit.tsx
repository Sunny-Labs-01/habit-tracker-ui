...
export default function EditHabit({ open, onClose, habit }: EditHabitProps) {
  const { updateHabit } = useApi();

  const initialFrequency = cronToFrequencyOption(habit.frequency);

  const [name, setName] = useState(habit.name);
  const [description, setDescription] = useState(habit.description ?? "");
  const [frequency, setFrequency] = useState<FrequencyOption>(initialFrequency.option);
  const [customDays, setCustomDays] = useState<number[]>(initialFrequency.customDays);
  const [selectedColor, setSelectedColor] = useState<string>(habit.color ?? HABIT_COLORS[0]);
  const [selectedEmoji, setSelectedEmoji] = useState(habit.emoji ?? "✅");
  const [loading, setLoading] = useState(false);

  // Reset state whenever the habit prop changes (different habit opened)
  useEffect(() => {
    const freq = cronToFrequencyOption(habit.frequency);
    setName(habit.name);
    setDescription(habit.description ?? "");
    setFrequency(freq.option);
  ...
  return (
    ...
    >
      <DialogTitle sx={{ pb: 1 }}>Edit Habit</DialogTitle>
      <DialogContent>
